...
def inject_css() -> None:
    ...
    [data-testid="stTabs"] [role="tablist"] {
        ...
        border: 1px solid #E0E8F5; padding: 4px;
    }
    ...
    [data-testid="stTabs"] [aria-selected="true"] {
        background: #1B3A8C !important; color: #FFFFFF !important; border-radius: 8px;
    }
...
def render_tab_ceo(data: dict) -> None:
    ...
    with col_right:
        ...
        def get_radar_vals(yr):
            ...
            r = roe[roe["年份"] == yr]
            sf = p["销售费用率"].values[0]
            sf = float(sf) if sf is not None and not (isinstance(sf, float) and np.isnan(sf)) else                  float(profit[profit["年份"]=="2024"]["销售费用率"].values[0])
            return [
...
def render_tab_product(data: dict) -> None:
    ...
    with col2:
        ...
        fig_growth.add_vline(x=overall_growth, line_dash="dot", line_color=COLORS["primary"], line_width=2,
                             annotation_text=f"整体增速 {overall_growth:.1f}%",
                             annotation_position="top right",
                             annotation_font=dict(color=COLORS["primary"], size=11))
        fig_growth.update_layout(**base_layout(title="产品线同比增速（FY2025 vs FY2024）", height=340),
                                 showlegend=False)
    ...
    fig_dual.update_layout(**base_layout(title="产品线营业收入 & 毛利率", height=400,
                                         legend=dict(orientation="h", y=-0.18, x=0.5, xanchor="center",
    ...
    fig_vol.update_layout(**base_layout(title="主要产品产销量（PCS）", height=400, barmode="group",
                                        legend=dict(orientation="h", y=-0.18, x=0.5, xanchor="center",
                                                    bgcolor="rgba(255,255,255,0.8)",
                                                    bordercolor="#E0E8F5", borderwidth=1)))
...
def render_tab_market(data: dict) -> None:
    ...
    with col1:
        ...
        fig_region.update_layout(**base_layout(title="分地区营业收入（FY2025）", height=340), showlegend=False)
    ...
    overall_growth = 21.77
    growth_colors = [COLORS["success"] if v >= overall_growth else COLORS["primary"]
    ...
    fig_reg_g.add_vline(x=overall_growth, line_dash="dot", line_color=COLORS["primary"], line_width=2,
                        annotation_text=f"整体增速 {overall_growth:.1f}%",
                        annotation_position="top right",
                        annotation_font=dict(color=COLORS["primary"], size=11))
    fig_reg_g.update_layout(**base_layout(title="分地区同比增速（FY2025 vs FY2024）", height=340), showlegend=False)
...
def render_tab_rd(data: dict) -> None:
    ...
    with col1:
        ...
        fig_exp.update_layout(**base_layout(title="三大费用两年对比（万元）", height=400, barmode="group",
                                            legend=dict(orientation="h", y=-0.18, x=0.5, xanchor="center",
                                                        bgcolor="rgba(255,255,255,0.8)",
                                                        bordercolor="#E0E8F5", borderwidth=1)))
    ...
    with col2:
        rev_growth = 21.77
        exp_colors = [COLORS["danger"] if v > rev_growth else COLORS["success"]
        ...
        fig_exp_g.add_hline(y=rev_growth, line_dash="dot", line_color=COLORS["primary"], line_width=2,
                            annotation_text=f"收入增速 {rev_growth:.1f}%",
                            annotation_position="top right",
                            annotation_font=dict(color=COLORS["primary"], size=11))
        fig_exp_g.update_layout(**base_layout(title="费用增速 vs 收入增速（%）", height=400), showlegend=False)
    ...
    fig_rd.update_layout(**base_layout(title="研发费用 & 研发费用率（三年趋势）", height=420,
                                       legend=dict(orientation="h", y=-0.18, x=0.5, xanchor="center",
                                                   bgcolor="rgba(255,255,255,0.8)",
                                                   bordercolor="#E0E8F5", borderwidth=1)))
